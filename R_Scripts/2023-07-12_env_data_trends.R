# environmental data time series trend analysis
# RJH
# 2023-07-12
...
mims1 <- read.csv('o2bio.csv') # this file is produced by read_lvm.py, refer to that script for details
...
sccoos.hourly$time <- parse_date_time(sccoos.hourly$time, orders = "Ymd HMS")
ggplot(sccoos.hourly) +
  geom_point(aes(x = time, y = temperature.sccoos)) +
  geom_smooth(aes(x = time, y = temperature.sccoos), method = "lm", se = F) +
  theme_bw()

my.lm <- lm(sccoos.hourly$temperature.sccoos~sccoos.hourly$time)
summary(my.lm)
