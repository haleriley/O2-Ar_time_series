...
sccoos3<- read.csv('https://erddap.sccoos.org/erddap/tabledap/autoss.csv?station%2Ctime%2Ctemperature%2Cconductivity%2Cpressure%2Cchlorophyll%2Csalinity&station=%22scripps_pier%22&time%3E=2017-12-10T00%3A00%3A00Z&time%3C=2023-12-17T05%3A08%3A00Z') # file will not download if it is too big
...
# mims2$date_time <- parse_date_time(mims2$time, orders = "Ymd HMS")
mims2 <- mims2[,-1]
...
mims2.merged$BOU <- ((((mims2.merged$O2.Ar * mims2.merged$O2_CF) / mims2.merged$O2.Ar_sat) - 1) * mims2.merged$O2_sat * 1) # assuming [Ar]/[Ar]sat = 1
...
saveRDS(combined.df, file = "2024-03-26_combined_env_data_hourly.rds")
...
saveRDS(combined.df.daily, file = "2024-03-26_combined_env_data_daily.rds")


# ---- nice plot of available AOU and BOU data -----

combined.df <- readRDS(file = "2024-03-26_combined_env_data_hourly.rds")
...
saveRDS(combined.df.select, file = "2024-03-26_combined.df.select.rds")
combined.df.select <- readRDS("2024-03-26_combined.df.select.rds")
...
m0.perc.err.test <- mean(abs(combined.df.test$aou - combined.df.test$BOU)/abs(combined.df.test$BOU))
...
m1.perc.err.test <- mean(abs(BOU.est - combined.df.test$BOU)/abs(combined.df.test$BOU))
...
saveRDS(final.gbm, file = "2024-03-26_final_gbm.rds")

# ---- apply final model to test data ----

final.gbm <- readRDS("2024-03-26_final_gbm.rds")
...
m2.perc.err.test <- mean(abs(BOU.est.final - combined.df.test$BOU)/abs(combined.df.test$BOU))
...
m3.perc.err.test <- mean(abs(BOU.est.final - combined.df.select$BOU)/abs(combined.df.select$BOU))
...
my.colnames <- colnames(combined.df)[-which(colnames(combined.df) == "Date.Time")]
combined.df.daily <- combined.df %>% group_by(Date) %>% summarize_all(mean)
saveRDS(combined.df.daily, file = "2024-04-01_combined_BOUest_env_daily.rds")
...
saveRDS(my.cross.val.values, file = "2024-04-11_cross_validation_results.rds")
my.cross.val.values <- readRDS("2024-04-11_cross_validation_results.rds")
