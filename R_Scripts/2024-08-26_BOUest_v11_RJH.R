...
miniDot.hourly <- rbind(NA, miniDot.hourly)
miniDot.hourly <- miniDot.hourly[,-4]

write.csv(miniDot.hourly, file = "2024-08-27_miniDOT_hourly_supplemental.csv")
...
options(timeout = 300)
sccoos <- read.csv("https://erddap.sensors.axds.co/erddap/tabledap/scripps-pier-automated-shore-sta-1.htmlTable?time%2Cmass_concentration_of_chlorophyll_in_sea_water_ctd%2Csea_water_electrical_conductivity_ctd%2Csea_water_practical_salinity_ctd%2Csea_water_pressure_ctd%2Csea_water_temperature_ctd%2Cstation&time%3E=2025-01-05T21%3A08%3A00Z")
...
sccoos.daily <- sccoos.daily %>% group_by(sample.date) %>%
  summarize(temperature.sccoos = mean(temperature), conductivity = mean(conductivity), pressure = mean(pressure), chlorophyll = mean(chlorophyll), salinity = mean(salinity))
saveRDS(sccoos.daily, file = "2024-07-24_sccoos_env_data_daily_mean.rds")
...
pdf("../Figures/manuscript_figures/2024-08-26_fig1.pdf", width = 10, height = 5)
...
t.test(combined.df$BOU[which(is.na(combined.df$BOU) == F)], combined.df$aou)
...
plot.3b <- ggplot() +
  geom_point(aes(x = combined.df.test$BOU, y = combined.df.test$aou, color = "-AOU"), size = 2, alpha = 0.7) +
  geom_point(aes(combined.df.test$BOU, y = BOU.est.final, color = "Estimated BOU"), size = 2, alpha = 0.7) +
...
pdf("../Figures/manuscript_figures/2024-08-26_fig2.pdf", width = 10, height = 5)

(plot.2a) / (plot.2b + plot.2c)

dev.off()

pdf("../Figures/manuscript_figures/2024-08-26_fig3.pdf", width = 10, height = 3)

plot.3a + plot.3b + plot_layout(widths = c(2,1))
...
saveRDS(combined.df, "2024-08-26_bou_est_df.rds")
...
my.colnames <- colnames(combined.df)[-which(colnames(combined.df) == "Date.Time")]
combined.df.daily <- combined.df %>% group_by(Date) %>% summarize_all(mean)
saveRDS(combined.df.daily, file = "2024-08-26_combined_BOUest_env_daily.rds")
...
pdf("../Figures/manuscript_figures/2024-08-26_fig5.pdf", width = 10, height = 5)
...
saveRDS(my.cross.val.values, file = "2024-08-26_cross_validation_results.rds")
my.cross.val.values <- readRDS("2024-08-26_cross_validation_results.rds")
...
plot.4c <- ggplot(data = combined.df) +
  geom_histogram(aes(log10(abs(error.c))), color = "black", fill = "#e6ab02") +
  theme_bw() +
  labs(x = "Log Relative Error [%]", y = "Frequency") +
  theme(axis.title = element_text(size = 14, face = "bold"), axis.text = element_text(size = 12), legend.title = element_text(size = 14, face = "bold"), legend.text = element_text(size = 12)) +
  theme(strip.background = element_rect(fill = "white", color = NULL), strip.text = element_text(size = 12, face = "bold")) +
  theme(axis.title = element_text(size = 14, face = "bold"), axis.text = element_text(size = 12)) +
  theme(legend.title = element_text(size = 14, face = "bold"), legend.text = element_text(size = 12)) +
...
plot.4d <- ggplot(df1, aes(x=PC1, y=PC2)) + 
  ...
  # scale_color_manual(values = c("grey69", "blue")) +
  # scale_alpha_manual(values = c(0.1, 0.8)) +
  scale_color_viridis_c(na.value = "grey69") +
...
plot.4e <- ggplot(df1, aes(x=PC1, y=PC2)) + 
  geom_point(aes(color = log10(abs(error.c)), alpha = na.alpha)) +
  geom_hline(yintercept=0, linetype="dotted") +
  geom_vline(xintercept=0, linetype="dotted") +
  # coord_fixed() +
  geom_segment(data=df2, aes(x=0, xend=PC1, y=0, yend=PC2),
...
plot.4b <- ggplot(df1, aes(x=PC1, y=PC2)) + 
  geom_point(aes(color = log10(abs(my.perc.error*100)), alpha = na.alpha), size = 2) +
  geom_hline(yintercept=0, linetype="dotted") +
  geom_vline(xintercept=0, linetype="dotted") +
  # coord_fixed() +
  geom_segment(data=df2, aes(x=0, xend=PC1, y=0, yend=PC2),
...
pdf("../Figures/manuscript_figures/2024-08-26_fig4.pdf", width = 13, height = 5)

(plot.4a + plot.4b) 

dev.off()

pdf("../Figures/manuscript_figures/2024-08-26_fig6.pdf", width = 13, height = 5)
