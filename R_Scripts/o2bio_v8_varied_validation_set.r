...
#sccoos <- read.csv('https://erddap.sccoos.org/erddap/tabledap/autoss.csv?station%2Ctime%2Cconductivity%2Cpressure%2Cchlorophyll%2Csalinity&station=%22scripps_pier%22&time%3E=2018-01-01T00%3A00%3A00Z&time%3C=2022-09-28T16%3A54%3A00Z')
...
mims.df <- as.data.frame(mims.zoo)
miniDot.df <- as.data.frame(miniDot.zoo)
sccoos.df <- as.data.frame(sccoos.zoo)
ljac.df <- as.data.frame(ljac.zoo)
ljpc1.df <- as.data.frame(ljpc1.zoo)

combined.df <- merge(mims.df, miniDot.df, by = "Group.1", all = TRUE)
combined.df <- merge(combined.df, sccoos.df, by = "Group.1", all = TRUE)
combined.df <- merge(combined.df, ljac.df, by = "Group.1", all = TRUE)
combined.df <- merge(combined.df, ljpc1.df, by = "Group.1", all = TRUE)

# combined.df <- as.data.frame(combined.df)
my.dates <- strptime(combined.df$Group.1, format = '%Y-%m-%d %H')
combined.df <- as.data.frame(lapply(combined.df[], as.numeric))
combined.df$dates <- my.dates


colnames(combined.df)[which(colnames(combined.df) == "temperature.y")] <- "temperature.sccoos.zoo"
...
combined.df <- combined.df[combined.df$salinity > 33.3,]

# saveRDS(combined.df, file = "2022-10-10_combined_env_data_hourly.rds")
...
# my.colnames <- colnames(combined.df)[c(1:23, 25:27)]
...
# try.it <- combined.df[which(is.na(combined.df$o2_bio) == FALSE),]
...
date1 <- strptime('2021-5-1', format = '%Y-%m-%d')
date2 <- strptime('2021-5-15', format = '%Y-%m-%d')
...
plot(full.predictors$dates[(smooth.ends + 1):(dim(full.predictors)[1] - smooth.ends)], full.aou.corrected.smooth, type = 'l', col = "orange",
     ylab = expression("AOU  ["*mu*"M]"),
     xlab = "Date")
